...
**Live version:** v0.8.46 (deployed to gh-pages, commit `37d2097` on `main` / `e103350` on `gh-pages`). The per-mode builder redesign (Phases A–E) is **complete**, print setup is a real paginated preview, and this session added: real Markdown rich text for faction backstory/notes, an opt-in "load example factions" offer, and per-era premade starter-list data (not yet wired into a screen).
...
- **Routing (hash):** `#/fleets`, `#/list/:id` (builder), `#/ships` (compendium), `#/solo`, `#/foundry` (Custom Rules), `#/play/:id`, `#/print/:id`, `#/learn[/:tab]` (Learn to Play), `#/learn-classic[/:step[/:anchor]]` (the archived walkthrough, unlinked). Share links are NOT routes: `#s=`/`#z=` payloads are intercepted at boot by `tryImportShare()`.
